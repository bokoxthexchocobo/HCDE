...
- **Predator mode (#12)** — survival-based asymmetric hunt: one
  server-assigned predator (stealth until fire/kill), survivors with one
  life per round (no respawn), Aliens-style radar, roaming monsters that
  **do** respawn, kill-earned currency with a predator-kill bonus. Late
  joiners spectate until the round ends. Phase 1/2 scaffold landed
  default-off. See `docs/HCDE_PREDATOR_AUDIT.md` and
