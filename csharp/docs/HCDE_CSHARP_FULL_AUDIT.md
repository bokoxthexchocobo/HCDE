...
**Verification:** `dotnet build` and `dotnet test` in `csharp/` — **415 tests passing** (CI: `.github/workflows/csharp.yml`; optional soak: `.github/workflows/csharp-cross-language-soak.yml`)  
...
The C# tree is a **well-tested protocol and networking foundation** (~15,500 LOC source, 415 unit/integration tests) covering Phase 1 tools, Phase 2a–2c wire codecs, `HCDE.Server` live authority pump with HCDW/HCDA/HCDS/HCIV/HCAV tail shipping, BEHAVIOR bytecode walk, and Phase 2d unified binary map decode.
