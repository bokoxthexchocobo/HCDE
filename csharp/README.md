...
| Map loader | `maploader/`, `p_setup.cpp` | `HCDE.MapLoader` | In progress (binary lump + geometry + surface decode, map-load bootstrap) |
...
Managed wire compatibility is gated by `dotnet test` (209 tests; CI via `.github/workflows/csharp.yml`). Optional cross-language soak CI runs `FullyQualifiedName~CrossLanguageSoak` when `HCDE_HCDESERV_PATH` / `HCDE_IWAD_PATH` secrets are configured. Cross-language checks live under `validation/`:
