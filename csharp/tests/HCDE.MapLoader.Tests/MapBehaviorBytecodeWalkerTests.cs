...
public class MapBehaviorBytecodeWalkerTests
{
    ...
    public void TryWalkScript_OldFormat_ReadsActorPitchStateAndEternityShiftOps()
    {
        ...
        Assert.Equal((int)AcsPcode.Dup, instructions[4].Opcode);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsScriptArrayShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                320, 1, // PCD_LSSCRIPTARRAY wire (313-324 shift block)
        ...
        Assert.True(terminated);
        Assert.Equal(8, instructions.Count);
        Assert.Equal(320, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsMapArrayShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                317, 1, // PCD_LSMAPARRAY wire (313-324 shift block)
                397, 2, // PCD_LSMAPARRAY wire (shadow LsMapArray enum alias)
                404, 3, // PCD_RSMAPARRAY wire (shadow RsMapArray enum alias)
                359, 4, // PCD_PUSHFUNCTION wire
                361, // PCD_SCRIPTWAITNAMED wire
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsWorldArrayShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                318, 1, // PCD_LSWORLDARRAY wire (313-324 shift block)
        ...
        Assert.True(terminated);
        Assert.Equal(8, instructions.Count);
        Assert.Equal(318, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsWorldVarShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                323, 1, // PCD_LSWORLDVAR wire (313-324 shift block)
                395, 2, // PCD_LSWORLDVAR wire (shadow LsWorldVar enum alias)
                402, 3, // PCD_RSWORLDVAR wire (shadow RsWorldVar enum alias)
                359, 4, // PCD_PUSHFUNCTION wire
                361, // PCD_SCRIPTWAITNAMED wire
                360, // PCD_CALLSTACK wire
                363, // PCD_GOTOSTACK wire
        ...
        Assert.True(terminated);
        Assert.Equal(8, instructions.Count);
        Assert.Equal(323, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
        Assert.Equal(395, instructions[1].Opcode);
        Assert.Equal(1, instructions[1].OperandWordCount);
        Assert.Equal(402, instructions[2].Opcode);
        Assert.Equal(1, instructions[2].OperandWordCount);
        Assert.Equal(359, instructions[3].Opcode);
        Assert.Equal(1, instructions[3].OperandWordCount);
        Assert.Equal(361, instructions[4].Opcode);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsWorldVarShiftFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
    ...
    [Fact]
    public void TryWalkScript_LittleEnhanced_ReadsWorldVarShiftFollowUpAndEternityStackOps()
    {
        var bytecode = new byte[]
        {
            240, 155, 1, // PCD_LSWORLDVAR wire (shadow LsWorldVar enum alias)
            240, 162, 2, // PCD_RSWORLDVAR wire (shadow RsWorldVar enum alias)
            (byte)AcsPcode.Terminate,
        };
        var lump = TestWadBuilder.BuildBehaviorLumpWithBytecode(
            MapBehaviorFormat.AcsLittleEnhanced,
        ...
        Assert.True(terminated);
        Assert.Equal(3, instructions.Count);
        Assert.Equal(395, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
        Assert.Equal(402, instructions[1].Opcode);
        Assert.Equal(1, instructions[1].OperandWordCount);
    }

    [Fact]
    public void TryWalkScript_OldFormat_ReadsGlobalVarShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                324, 1, // PCD_LSGLOBALVAR wire (313-324 shift block)
                396, 2, // PCD_LSGLOBALVAR wire (shadow LsGlobalVar enum alias)
                403, 3, // PCD_RSGLOBALVAR wire (shadow RsGlobalVar enum alias)
                359, 4, // PCD_PUSHFUNCTION wire
                361, // PCD_SCRIPTWAITNAMED wire
                360, // PCD_CALLSTACK wire
                363, // PCD_GOTOSTACK wire
        ...
        Assert.True(terminated);
        Assert.Equal(8, instructions.Count);
        Assert.Equal(324, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
        Assert.Equal(396, instructions[1].Opcode);
        Assert.Equal(1, instructions[1].OperandWordCount);
        Assert.Equal(403, instructions[2].Opcode);
        Assert.Equal(1, instructions[2].OperandWordCount);
        Assert.Equal(359, instructions[3].Opcode);
        Assert.Equal(1, instructions[3].OperandWordCount);
        Assert.Equal(361, instructions[4].Opcode);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsGlobalVarShiftFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
    ...
    [Fact]
    public void TryWalkScript_LittleEnhanced_ReadsGlobalVarShiftFollowUpAndEternityStackOps()
    {
        var bytecode = new byte[]
        {
            240, 156, 1, // PCD_LSGLOBALVAR wire (shadow LsGlobalVar enum alias)
            240, 163, 2, // PCD_RSGLOBALVAR wire (shadow RsGlobalVar enum alias)
            (byte)AcsPcode.Terminate,
        };
        var lump = TestWadBuilder.BuildBehaviorLumpWithBytecode(
            MapBehaviorFormat.AcsLittleEnhanced,
        ...
        Assert.True(terminated);
        Assert.Equal(3, instructions.Count);
        Assert.Equal(396, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
        Assert.Equal(403, instructions[1].Opcode);
        Assert.Equal(1, instructions[1].OperandWordCount);
    }

    [Fact]
    public void TryWalkScript_OldFormat_ReadsMapVarShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                322, 1, // PCD_LSMAPVAR wire (313-324 shift block)
        ...
        Assert.True(terminated);
        Assert.Equal(8, instructions.Count);
        Assert.Equal(322, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsGlobalArrayShiftBlockEntryFollowUpAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                319, 1, // PCD_LSGLOBALARRAY wire (313-324 shift block)
        ...
        Assert.True(terminated);
        Assert.Equal(8, instructions.Count);
        Assert.Equal(319, instructions[0].Opcode);
        Assert.Equal(1, instructions[0].OperandWordCount);
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsShiftBlockCompletionCrossOpFollowUpsAndEternityStackOps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            bytecodeOpcodes:
            [
                321, 1, // PCD_LSSCRIPTVAR wire (313-324 shift block completion)
                376, 2, // PCD_LSSCRIPTARRAY wire (cross-op script-array shadow)
                377, 3, // PCD_RSSCRIPTARRAY wire (cross-op script-array shadow)
                322, 4, // PCD_LSMAPVAR wire
                397, 5, // PCD_LSMAPARRAY wire (cross-op map-array shadow)
                404, 6, // PCD_RSMAPARRAY wire (cross-op map-array shadow)
                323, 7, // PCD_LSWORLDVAR wire
                398, 8, // PCD_LSWORLDARRAY wire (cross-op world-array shadow)
                405, 9, // PCD_RSWORLDARRAY wire (cross-op world-array shadow)
                324, 10, // PCD_LSGLOBALVAR wire
                399, 11, // PCD_LSGLOBALARRAY wire (cross-op global-array shadow)
                406, 12, // PCD_RSGLOBALARRAY wire (cross-op global-array shadow)
                359, 13, // PCD_PUSHFUNCTION wire
                361, // PCD_SCRIPTWAITNAMED wire
                360, // PCD_CALLSTACK wire
                363, // PCD_GOTOSTACK wire
                (int)AcsPcode.Terminate,
    ...
    [Fact]
    public void TryWalkScript_OldFormat_ReadsShiftBlockCompletionEternityStackCrossOpFollowUps()
    {
        var lump = TestWadBuilder.BuildBehaviorLump(
            MapBehaviorFormat.AcsOld,
            scriptCount: 1,
            ...
            [
                ...
                360, // PCD_CALLSTACK wire
                363, // PCD_GOTOSTACK wire
                321, 14, // PCD_LSSCRIPTVAR wire (Eternity stack cross-op follow-up)
                376, 15, // PCD_LSSCRIPTARRAY wire (Eternity stack cross-op follow-up)
                377, 16, // PCD_RSSCRIPTARRAY wire (Eternity stack cross-op follow-up)
                322, 17, // PCD_LSMAPVAR wire (Eternity stack cross-op follow-up)
                (int)AcsPcode.Terminate,
            ]);
        Assert.True(MapBehaviorCodec.TryProbe(lump, out var record, out _));
        Assert.True(MapBehaviorDirectoryCodec.TryReadScripts(
        ...
        Assert.True(terminated);
        Assert.Equal(21, instructions.Count);
        Assert.Equal(321, instructions[0].Opcode);
        Assert.Equal(376, instructions[1].Opcode);
        Assert.Equal(377, instructions[2].Opcode);
        Assert.Equal(322, instructions[3].Opcode);
        ...
        Assert.Equal(360, instructions[14].Opcode);
        Assert.Equal(363, instructions[15].Opcode);
        Assert.Equal(321, instructions[16].Opcode);
        Assert.Equal(1, instructions[16].OperandWordCount);
        Assert.Equal(376, instructions[17].Opcode);
        Assert.Equal(1, instructions[17].OperandWordCount);
        Assert.Equal(377, instructions[18].Opcode);
        Assert.Equal(1, instructions[18].OperandWordCount);
        Assert.Equal(322, instructions[19].Opcode);
        Assert.Equal(1, instructions[19].OperandWordCount);
    }
    ...
    [Fact]
    public void TryWalkScript_LittleEnhanced_ReadsShiftBlockCompletionEternityStackCrossOpFollowUps()
    {
        var bytecode = new byte[]
        {
            240, 153, 1, // PCD_LSSCRIPTVAR wire (cross-op script-var entry)
            ...
            240, 120, // PCD_CALLSTACK wire
            240, 123, // PCD_GOTOSTACK wire
            240, 153, 14, // PCD_LSSCRIPTVAR wire (Eternity stack cross-op follow-up)
            240, 136, 15, // PCD_LSSCRIPTARRAY wire (Eternity stack cross-op follow-up)
            240, 137, 16, // PCD_RSSCRIPTARRAY wire (Eternity stack cross-op follow-up)
            240, 154, 17, // PCD_LSMAPVAR wire (Eternity stack cross-op follow-up)
            (byte)AcsPcode.Terminate,
        };
        var lump = TestWadBuilder.BuildBehaviorLumpWithBytecode(
            MapBehaviorFormat.AcsLittleEnhanced,
        ...
        Assert.Equal(360, instructions[14].Opcode);
        Assert.Equal(363, instructions[15].Opcode);
        Assert.Equal(393, instructions[16].Opcode);
        Assert.Equal(1, instructions[16].OperandWordCount);
        Assert.Equal(376, instructions[17].Opcode);
        Assert.Equal(1, instructions[17].OperandWordCount);
        Assert.Equal(377, instructions[18].Opcode);
        Assert.Equal(1, instructions[18].OperandWordCount);
        Assert.Equal(394, instructions[19].Opcode);
        Assert.Equal(1, instructions[19].OperandWordCount);
    }
