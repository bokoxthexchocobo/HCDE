...
public class SnapshotChecksumPresentationEchoPolicyTests
{
    ...
    [Fact]
    public void PolishLineSpecRollingHash_MixesPresentationEchoTailWhenBothPresent()
    {
        var echoHash = SnapshotChecksumPresentationEchoPolicy.MixBlock(0, PresentationEchoCodec.CreateExampleBlock());
        var lineSpecHash = SnapshotChecksumLineSpecPolicy.MixRecord(0, lineIndex: 3, special: 5, success: true);
        var polished = SnapshotChecksumPresentationEchoPolicy.PolishLineSpecRollingHash(lineSpecHash, echoHash);

        Assert.NotEqual(lineSpecHash, polished);
        Assert.Equal(lineSpecHash, SnapshotChecksumPresentationEchoPolicy.PolishLineSpecRollingHash(lineSpecHash, presentationEchoHash: 0));
    }

    [Fact]
    public void PolishAuthorityEventRollingHash_MixesPresentationEchoTailWhenBothPresent()
    {
        var echoHash = SnapshotChecksumPresentationEchoPolicy.MixBlock(0, PresentationEchoCodec.CreateExampleBlock());
        var authorityHash = SnapshotChecksumAuthorityEventPolicy.MixRecord(0, AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 42));
        var polished = SnapshotChecksumPresentationEchoPolicy.PolishAuthorityEventRollingHash(authorityHash, echoHash);

        Assert.NotEqual(authorityHash, polished);
        Assert.Equal(authorityHash, SnapshotChecksumPresentationEchoPolicy.PolishAuthorityEventRollingHash(authorityHash, presentationEchoHash: 0));
    }

    [Fact]
    public void CommitAppliedActorDeltas_PolishesAuthorityEventRollingHashFromPresentationEcho()
    {
        var store = new GuestWorldStateStore();
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 42);
        store.CommitAppliedAuthorityEvents(new[] { authorityRecord });
    ...
    [Fact]
    public void CommitAppliedActorDeltas_PolishesActorDeltaRollingHashFromPresentationEcho()
    {
        var store = new GuestWorldStateStore();
        store.CommitAppliedPresentationEcho(PresentationEchoCodec.CreateExampleBlock());
    ...
    [Fact]
    public void PolishRollingHashWithAuthorityEvent_MixesAuthorityEventTailWhenBothPresent()
    {
        var echoHash = SnapshotChecksumPresentationEchoPolicy.MixBlock(0, PresentationEchoCodec.CreateExampleBlock());
        var authorityHash = SnapshotChecksumAuthorityEventPolicy.MixRecord(
            0,
            AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 8));
        var polished = SnapshotChecksumPresentationEchoPolicy.PolishRollingHashWithAuthorityEvent(echoHash, authorityHash);

        Assert.NotEqual(echoHash, polished);
        Assert.Equal(echoHash, SnapshotChecksumPresentationEchoPolicy.PolishRollingHashWithAuthorityEvent(echoHash, authorityEventHash: 0));
    }

    [Fact]
    public void CommitAppliedActorDeltas_PolishesPresentationEchoRollingHashFromAuthorityEvent()
    {
        var store = new GuestWorldStateStore();
        store.CommitAppliedPresentationEcho(PresentationEchoCodec.CreateExampleBlock());
        store.CommitAppliedAuthorityEvents(new[]
        {
            AuthorityEventsCodec.CreateSpawnExample("DoomImp", actorId: 17),
        });
        var before = store.PresentationEchoRollingHash;
