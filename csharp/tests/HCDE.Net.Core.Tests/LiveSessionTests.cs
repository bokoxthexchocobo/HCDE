...
public class LiveSessionTests
{
    ...
    [Fact]
    public void GuestPresentationEchoWithoutChecksum_TriggersNetGapResync()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        ...
        Assert.True(guest.TryReceiveServerSnapshot(out _, out _, out var tailSections));
        Assert.NotNull(tailSections);
        Assert.NotNull(tailSections!.Value.EchoBlock);
        Assert.False(tailSections.Value.HasChecksum);
        Assert.NotEqual(0u, guestStore.PresentationEchoRollingHash);
    ...
    [Fact]
    public void GuestInvasionLineSpecWithoutChecksum_TriggersNetGapResync()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 6, special: 12, success: true);
        var guest = new LiveGuestSession(
            guestTransport,
            gameId,
            authorityEndpoint,
    ...
    [Fact]
    public void GuestAppliesEmbeddedInvasionAuthorityEventsWhenWorldStateWired()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        ...
        var authorityStore = new GuestWorldStateStore();
        authorityStore.QueueAuthorityEvent(AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 91));
        var authority = new LiveAuthoritySession(authorityTransport, gameId, authoritySlot: 0, maxClients: 4);
        authority.SetAuthorityWorldState(authorityStore, new SnapshotChecksumSession());
        authority.SetAuthorityInvasionSnapshot(new InvasionSnapshotHeader(
            flags: 0,
        ...
        Assert.NotNull(tailSections.Value.AuthorityEventRecords);
        Assert.Single(tailSections.Value.AuthorityEventRecords!);
        Assert.NotNull(guest.AuthorityEventState);
        Assert.Equal(1, guest.AuthorityEventState!.RoutedCount);
        Assert.Equal(91u, guest.AuthorityEventState.LastActorId);
        Assert.NotEqual(0u, guestStore.AuthorityEventRollingHash);
    }

    [Fact]
    public void GuestInvasionCoopDeadSpawnsWithoutChecksum_TriggersNetGapResync()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        ...
        var written = ServerSnapshotTailCodec.WriteInvasionShipping(
            ...
            new InvasionSnapshotHeader(
                ...
                waveCleared: 0,
                activeMonsters: 2),
            coopDeadSpawnIndices: new uint[] { 88 });
    ...
    [Fact]
    public void GuestInvasionAuthorityEventsWithoutChecksum_TriggersNetGapResync()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
    ...
    [Fact]
    public void AuthorityPump_MergesCoopWorldDeltaIntoInvasionTail()
    {
        var wad = HCDE.MapLoader.Tests.TestWadBuilder.BuildMinimalMapWad("MAP01");
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        ...
        Assert.NotEmpty(tailSections.Value.WorldDeltaSectors!);
    }
    ...
    [Fact]
    public void GuestAppliesEmbeddedInvasionActorDeltasWhenWorldStateWired()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        ...
        var authorityStore = new GuestWorldStateStore();
        authorityStore.SeedActor(actorId: 44, classId: 7, health: 55);
        var authority = new LiveAuthoritySession(authorityTransport, gameId, authoritySlot: 0, maxClients: 4);
        authority.SetAuthorityWorldState(authorityStore, new SnapshotChecksumSession());
        authority.SetAuthorityInvasionSnapshot(new InvasionSnapshotHeader(
            flags: 0,
        ...
        Assert.NotNull(tailSections!.Value.InvasionSnapshot);
        Assert.NotNull(tailSections.Value.ActorDeltaRecords);
        Assert.Single(tailSections.Value.ActorDeltaRecords!);
        Assert.True(guestStore.Actors.ContainsKey(44));
        Assert.Equal(55, guestStore.Actors[44].Health);
        Assert.NotEqual(0u, guestStore.ActorDeltaRollingHash);
    }
    ...
    [Fact]
    public void GuestInvasionPresentationEchoWithoutChecksum_TriggersNetGapResync()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
