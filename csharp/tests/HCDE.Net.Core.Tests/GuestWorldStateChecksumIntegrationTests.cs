...
public class GuestWorldStateChecksumIntegrationTests
{
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnInvasionAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 44;
        const int rngSeed = 42;
        const uint deadSpawnIndex = 96;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 108);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 14, special: 17, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 14, special: 17, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
        ...
        Assert.True(guest.LastChecksumApplyState.HasActorCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.HasLineSpecCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.MismatchCount >= 2);
        Assert.True(guest.NeedsChecksumResync);
        Assert.True(guest.NeedsNetGapResync);
        Assert.Equal(0, guest.NetRegistry.GetOrCreate(1).CurrentSequence);
        Assert.Contains(deadSpawnIndex, guestStore.RetiredCoopDeadSpawns);
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnInvasionAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketFollowUpChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 46;
        const int rngSeed = 44;
        const uint deadSpawnIndex = 98;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 110);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 16, special: 19, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        remoteHashes[(int)SnapshotChecksumCategory.Actors] ^= 0xFFFF;
        remoteHashes[(int)SnapshotChecksumCategory.LineSpec] ^= 0xFFFF;
        remoteHashes[(int)SnapshotChecksumCategory.Sectors] ^= 0xFFFF;
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 16, special: 19, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
        ...
        Assert.True(guest.LastChecksumApplyState.HasActorCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.HasLineSpecCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.MismatchCount >= 3);
        Assert.True(guest.NeedsChecksumResync);
        Assert.True(guest.NeedsNetGapResync);
        Assert.Equal(0, guest.NetRegistry.GetOrCreate(1).CurrentSequence);
        Assert.Contains(deadSpawnIndex, guestStore.RetiredCoopDeadSpawns);
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnInvasionAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketFollowUpFollowUpChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 48;
        const int rngSeed = 46;
        const uint deadSpawnIndex = 100;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 112);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 18, special: 21, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        remoteHashes[(int)SnapshotChecksumCategory.LineSpec] ^= 0xFFFF;
        remoteHashes[(int)SnapshotChecksumCategory.Sectors] ^= 0xFFFF;
        remoteHashes[(int)SnapshotChecksumCategory.Players] ^= 0xFFFF;
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 18, special: 21, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
        ...
        Assert.True(guest.LastChecksumApplyState.HasActorCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.HasLineSpecCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.MismatchCount >= 4);
        Assert.True(guest.NeedsChecksumResync);
        Assert.True(guest.NeedsNetGapResync);
        Assert.Equal(0, guest.NetRegistry.GetOrCreate(1).CurrentSequence);
        Assert.Contains(deadSpawnIndex, guestStore.RetiredCoopDeadSpawns);
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnCoopAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketFollowUpFollowUpFollowUpChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 49;
        const int rngSeed = 47;
        const uint deadSpawnIndex = 101;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 113);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 19, special: 22, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        remoteHashes[(int)SnapshotChecksumCategory.Sectors] ^= 0xFFFF;
        remoteHashes[(int)SnapshotChecksumCategory.Players] ^= 0xFFFF;
        remoteHashes[(int)SnapshotChecksumCategory.Rng] ^= 0xFFFF;
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 19, special: 22, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
        ...
        Assert.True(guest.LastChecksumApplyState.HasActorCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.HasLineSpecCategoryMismatch);
        Assert.True(guest.LastChecksumApplyState.MismatchCount >= 5);
        Assert.True(guest.NeedsChecksumResync);
        Assert.True(guest.NeedsNetGapResync);
        Assert.Equal(0, guest.NetRegistry.GetOrCreate(1).CurrentSequence);
        Assert.Contains(deadSpawnIndex, guestStore.RetiredCoopDeadSpawns);
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnCoopAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketFollowUpFollowUpChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 47;
        const int rngSeed = 45;
        const uint deadSpawnIndex = 99;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 111);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 17, special: 20, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 17, special: 20, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnCoopAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 43;
        const int rngSeed = 41;
        const uint deadSpawnIndex = 95;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 107);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 13, special: 16, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 13, special: 16, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnCoopAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecMultiBucketFollowUpChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 45;
        const int rngSeed = 43;
        const uint deadSpawnIndex = 97;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 109);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 15, special: 18, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 15, special: 18, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnInvasionAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 42;
        const int rngSeed = 40;
        const uint deadSpawnIndex = 94;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 106);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 12, special: 15, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 12, special: 15, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnCoopAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnActorLineSpecChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 41;
        const int rngSeed = 39;
        const uint deadSpawnIndex = 93;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 105);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 11, special: 14, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 11, special: 14, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
    ...
    [Fact]
    public void GuestReceive_TriggersNetGapResyncOnInvasionAuthorityEventActorDeltaPresentationEchoCoopDeadSpawnLineSpecChecksumMismatchWhenPolicySet()
    {
        var gameId = new byte[] { 0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22, 0x33, 0x44 };
        const int gameTic = 40;
        const int rngSeed = 38;
        const uint deadSpawnIndex = 92;
        ...
        var authorityRecord = AuthorityEventsCodec.CreateSpawnExample("Imp", actorId: 104);
        var echoBlock = PresentationEchoCodec.CreateExampleBlock();

        var authorityStore = new GuestWorldStateStore();
        authorityStore.NoteLineSpec(lineIndex: 10, special: 13, success: true);
        authorityStore.TryApply(1, actor);
        authorityStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        authorityStore.CommitAppliedPresentationEcho(echoBlock);
        authorityStore.TryRetireSpawnIndex(deadSpawnIndex);
        ...
        var guestStore = new GuestWorldStateStore();
        guestStore.NoteLineSpec(lineIndex: 10, special: 13, success: true);
        guestStore.TryApply(1, actor);
        guestStore.CommitAppliedAuthorityEvents(new[] { authorityRecord });
        guestStore.CommitAppliedPresentationEcho(echoBlock);
        var guestSession = new SnapshotChecksumSession();
